        ...
        for (int i=0; i<timeList.size() && i<accelList.size(); i++){
            ...
            */
            ...
            if(queueAccel.size() == 3){
                saveVelo += distance.simpson3point(queueAccel,  time-timeList.get(i-2));
                queueVelocity.add(saveVelo);

                if (queueVelocity.size() == 3){
                    saveDist += distance.simpson3point(queueVelocity,  time-timeList.get(i-4));
                    estDist = saveDist;
                    for(int j=0;j<2;j++){
                        queueVelocity.remove();
                    }
    ...
    public void readCSV(){
        try {
            InputStream inputStream =
                    getResources().getAssets().open("acceldata.csv");
