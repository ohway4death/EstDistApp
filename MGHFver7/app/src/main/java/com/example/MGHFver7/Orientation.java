...
class Orientation {
    ...
    public static float q0 = 1f;
    public static float q1 = 0f;
    public static float q2 = 0f;
    public static float q3 = 0f;

   //論文では0.041
   //大きくした方がバイアスの補正をより短い間隔で行え，加速度に対して敏感になる(https://stackoverflow.com/questions/47589230/what-is-the-best-beta-value-in-madgwick-filter)
   float beta = 0.3f;
   ...
   public float[] madgwickFilter(float accel[], float gyro[], float mag[], float sampleFreq){
       ...
       qDot1 = 0.5f * (-q1 * gx - q2 * gy - q3 * gz); //Qw
       qDot2 = 0.5f * (q0 * gx + q2 * gz - q3 * gy);  //Qx
       qDot3 = 0.5f * (q0 * gy - q1 * gz + q3 * gx);  //Qy
       qDot4 = 0.5f * (q0 * gz + q1 * gy - q2 * gx);  //Qz
       ...
       */
       ...
       //https://www.kazetest.com/vcmemo/quaternion/quaternion.htm これを参考にしてクォータニオンからオイラー角に変換した、サンプルプログラムと同じ値を示している->計算式からもサンプルプログラムと同じになることを確認

       //回転順ZYX(https://qiita.com/aa_debdeb/items/3d02e28fb9ebfa357eaf)
       roll = (float) Math.toDegrees(Math.atan2(2 * (q0*q1 + q2*q3), q0*q0 - q1*q1 - q2*q2 + q3*q3));
       pitch = (float) Math.toDegrees(Math.asin(2 * (q0*q2 - q1*q3)));
       yaw = (float) Math.toDegrees(Math.atan2(2 * (q0*q3 + q1*q2), q0*q0 + q1*q1 - q2*q2 - q3*q3));
    ...
    public float[] madgwickFilterNoMag (float accel[], float gyro[], float sampleFreq){
       ...
       float invSampleFreq = 1.0f / sampleFreq;
        ...
        qDot0 = 0.5f * (-q1 * gx - q2 * gy - q3 * gz); //Qw
        qDot1 = 0.5f * (q0 * gx + q2 * gz - q3 * gy);  //Qx
        qDot2 = 0.5f * (q0 * gy - q1 * gz + q3 * gx);  //Qy
        qDot3 = 0.5f * (q0 * gz + q1 * gy - q2 * gx);  //Qz
       ...
       float roll = (float) Math.toDegrees(Math.atan2(2 * (q0*q1 + q2*q3), q0*q0 - q1*q1 - q2*q2 + q3*q3));
       float pitch = (float) Math.toDegrees(Math.asin(2 * (q0*q2 - q1*q3)));
       float yaw = (float) Math.toDegrees(Math.atan2(2 * (q0*q3 + q1*q2), q0*q0 + q1*q1 - q2*q2 - q3*q3));
